...
{
	internal struct IntermediateOperationActions
	{
		private readonly IMathContext context;

		private IMathContext Context
		{
			get
			{
				return context;
		...
		public MathValue Multiply(MathValue left, MathValue right)
		{
			...
			// right = ResolveNumericType (right);
			if ((left.ValueClass != right.ValueClass || left.ValueClass != ValueClassifier.Float)
				&& left.ValueClass != ValueClassifier.FloatFractional)
			{
